...
left join users as u on u.id = t.user_id where ";
	...
	public function searchHistory($bookId, $userId)
		...
		public function getBookTransactionsUser($now){
			$query = "SELECT DISTINCT * FROM `book_transactions` WHERE `return_plan_date` < '$now' AND `return_actual_date` IS NULL ";
			return $this->pdo->query($query)->fetchAll(PDO::FETCH_ASSOC);

		}
		public function getBookTransactionsBook(){
			$query = "SELECT book_transactions.book_id,books.quantity ,COUNT(book_transactions.book_id) AS countbook FROM book_transactions INNER JOIN books ON book_transactions.book_id = books.id GROUP BY book_id Having books.quantity < countbook";
			return $this->pdo->query($query)->fetchAll(PDO::FETCH_ASSOC);
